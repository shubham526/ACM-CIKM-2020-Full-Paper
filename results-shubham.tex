...
However, \ld{"Many passages not contain any detected salient entities, yielding many empty results especially for sentence contexts" instead of} SWAT returns many empty results when asked for only salient entities than when asked for all (salient or otherwise) entities.
\ld{cut?} For example, using the sentence context of an entity mention, SWAT returns an empty result for 100 of the 201 entity mentions when asked for only the salient entities and 13 of 201 entity mentions when asked for all  entities. 
...
To study the quality of the entity ranking for aspect matching, we train a L2R model to learn the weighted combination for these alternatives:
...
\subsubsection{RQ? Frequency vs Relatedness}
